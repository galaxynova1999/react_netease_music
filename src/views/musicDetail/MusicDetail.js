...
class MusicDetail extends React.Component {
  ...
  handleComment() {
    ...
    sendComment("0", 1, this.props.id, this.state.myComment)
      .then(function (res) {
        createSuccess("评论成功!");
      })
      .catch(function (err) {
  ...
  SimiSong() {
    return (
      <div>
        <p style={{ fontSize: "18px", margin: "10px 0 10px 20px" }}>相似歌曲</p>
        <Divider />
  ...
  SimiPlayList() {
    return (
      <div style={{ marginTop: "15px" }}>
        <p style={{ fontSize: "18px", margin: "10px 0 10px 20px" }}>
          包含这首歌的歌单
        </p>
        <Divider />
          ...
          return (
            ...
            >
              ...
              >
                ...
                <p style={{ fontSize: "0.8em" }}>播放: {item.playCount}</p>
              </div>
            </div>
          );
        })}
  ...
  handleCollect(id) {
    ...
    addNewSongToPlayList(id, this.state.id)
      .then((res) => {
        createSuccess("收藏成功!");
      })
      .catch(function (err) {
        ...
        <div className={styles.top}>
              ...
              >
                ...
                >
                  {userPlayList.playList &&
                    userPlayList.playList.map((item, index) => {
                      return (
                        <div
          ...
          <div className={styles.topRight}>
            ...
            {!detailState.data.loading && (
              <Lyric
                lyric={detailState.data.lyric}
                tlyric={detailState.data.tlyric}
              />
            )}
          </div>
        </div>
          ...
          <div className={styles.writeComment}>
            <p style={{ marginLeft: "20px" }}>
              ...
              <span style={{ marginLeft: "10px" }}>
                已有{detailState.data.totalCount}条评论
              </span>
            </p>
            <TextField
              label="评论"
