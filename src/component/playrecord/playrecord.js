...
function UserPlayRecordAndPlayHistory() {
  ...
  function playList() {
    ...
    if (user_list.length === 0) {
      return <div className={style.no_list}>暂无播放列表</div>;
    } else {
      return (
        <TableContainer>
          <Table style={{ width: "500px" }} size="small">
            ...
            <TableBody>
              {user_list.map((row, index) => (
                <TableRow
                  key={row.song.id}
                  onDoubleClick={() => handleSelect(row.song.id)}
                  hover={true}
                >
                  <TableCell>
                  ...
                  <TableCell align="right">
                    {row.song.ar.length > 1
                      ? row.song.ar[0].name + "/" + row.song.ar[1].name
                      : row.song.ar[0].name}
                  </TableCell>
  ...
  function playHistory() {
    ...
    if (user_hs === null) {
      return <div className={style.no_list}>暂无播放历史</div>;
    } else {
      return (
        <TableContainer>
          <Table style={{ width: "500px" }} size="small">
