        ...
        trans.forEach(function (item) {
            ...
            if(w.indexOf("[")!==-1){
                ...
                if(ww==="")
                    return;
                tlyric.push({
                    time:(tt.split(":")[0]*60 + parseFloat(tt.split(":")[1])).toFixed(3),
                    word:ww
                })
            }
            else {
    ...
    timeAndword.forEach(function (item,index) {
        ...
        if(w.indexOf("[")!==-1){
            ...
            if(ww==="")
                return;
            let t_word;
            let time=(tt.split(":")[0]*60 + parseFloat(tt.split(":")[1])).toFixed(3)
            if(tlyric.length!==0){
...
function Lyric(props) {
  ...
  function LyricScroll() {
      ...
      const lineIndex = formattedLyric.findIndex((item,index,arr) => {
          const prev = index - 1 >= 0 ? parseFloat(arr[index - 1].time) : parseFloat(item.time);
          const next = index + 1 < arr.length ? parseFloat(arr[index + 1].time) : parseFloat(item.time);
      ...
      if(lineIndex > -1 && lineIndex !== currentLineIndex.current){
          ...
          if(div && div.scrollTop + div.clientHeight <= div.scrollHeight) {
              console.log(lineHeight,lineIndex,div.clientHeight,div.scrollHeight,div.scrollTop);
              const scrollHeight = lineHeight * lineIndex - (div.clientHeight * 0.60) ;
      ...
      currentRequestAnimationFrame.current = window.requestAnimationFrame(LyricScroll);
  }
  useEffect(() => {
