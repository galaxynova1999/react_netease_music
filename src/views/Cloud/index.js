...
function Cloud() {
  ...
  useEffect(() => {
    getMeCloud().then(function (res) {
      setData(res.data);
      setProgress(
        res.data.size / res.data.maxSize < 0.05
          ? (res.data.size / res.data.maxSize) * 100 + 10
          : (res.data.size / res.data.maxSize) * 100
      );
    });
  }, []);
    ...
    <div style={{ width: "100%", paddingLeft: "3vw", marginTop: "40px" }}>
      <div style={{ fontSize: "20px", marginBottom: "20px" }}>
        ...
        <span style={{ fontSize: "0.8em" }}>
          {bytesToSize(data.size)}/{bytesToSize(data.maxSize)}
        </span>
