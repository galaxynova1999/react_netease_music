...
class MySide extends React.Component {
  ...
  handleMeLike() {
    changeRouteTo(
      this.props,
      "/playlist/songlist/" + userPlayListState.userLikePlayListID,
      true
    );
  }
  handleMeCloud() {
  ...
  render() {
    ...
    return (
      <div style={{ width: "100%" }} className={style.main}>
        ...
        <NavLink className={style.NavLink} to="/discovery">
          <div
            className={classnames(
              style.ListItem,
              this.props.location.pathname.indexOf("/discovery") === 0
                ? style.active
                : style.deactive
            )}
          >
        ...
        <div
          onClick={this.handlePersonalFM.bind(this)}
          className={classnames(
            style.ListItem,
        ...
        <div
          className={classnames(
            style.ListItem,
            this.props.location.pathname === "/cloud"
              ? style.active
              : style.deactive
          )}
          onClick={this.handleMeCloud.bind(this)}
        >
        ...
        <div
          className={classnames(
            style.ListItem,
            this.props.location.pathname === "/collect"
              ? style.active
              : style.deactive
          )}
          onClick={this.handleMeCollect.bind(this)}
        >
        ...
        <p style={{ fontSize: "15px" }}>
          创建的歌单{" "}
          <AddCircleOutlineIcon style={{ float: "right", fontSize: "23px" }} />
        </p>
        <Divider />
        <div
          className={classnames(
          ...
          onClick={this.handleMeLike.bind(this)}
        >
          <FavoriteBorderIcon
            style={{ marginLeft: "10px", fontSize: "18px" }}
          />
        ...
        {userPlayListState.playList &&
          userPlayListState.playList.map(function (item, index) {
            return (
              <NavLink
                className={style.NavLink}
                to={"/playlist/songlist/" + item.id}
                key={index}
              >
                <div
                  className={classnames(
        ...
        <Divider />
        {userPlayListState.subscribedPlayList &&
          userPlayListState.subscribedPlayList.map(function (item, index) {
            return (
              <NavLink
                className={style.NavLink}
                to={"/playlist/songlist/" + item.id}
