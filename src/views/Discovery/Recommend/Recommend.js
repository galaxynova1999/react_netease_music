...
class Recommend extends React.Component {
  ...
  render() {
    const BannerData = this.state.banner.map((item, index) => {
      return (
        <div key={index} className={style.banner}>
          <img
            src={item.imageUrl + "?param=900y270"}
            alt="1"
            style={{
              borderRadius: "10px",
              maxWidth: "100%",
    ...
    const DailyRecommendPlayListData = this.state.playlist.map(
      (item, index) => {
        return (
          <div
            key={index}
      ...
      return (
        <div className={style.mv} key={index}>
          ...
          <div style={{ fontSize: "15x" }}>
            <p>{item.name}</p>
            <p style={{ color: "rgb(186,186,186)", fontSize: "0.9em" }}>
              {item.artistName}
            </p>
            <span
              style={{
              ...
              return (
                <div key={index} className={style.music}>
                  <img
                    ...
                    alt="1"
                    onClick={this.handleNewMusic.bind(this, item.id)}
                  />
                  <span
                    style={{
                      float: "left",
