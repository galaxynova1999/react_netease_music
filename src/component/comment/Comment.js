...
import {createError, createInfo, createSuccess} from "../notification/Notification";
...
class Comment extends React.Component{
    ...
    handleLikeComment(){
        if(!checkLogin())
            return;
       if(!this.state.liked) {
           likeComment(this.props.id,this.props.data.commentId,1,this.props.type).then( (res) => {
               this.setState({
                   liked:true,
           ...
           likeComment(this.props.id,this.props.data.commentId,0,this.props.type).then( (res) => {
               this.setState({
                   liked:false,
                   count:this.state.count - 1
               },() => {
        ...
        return(
            <div className={style.main} style={this.props.style}>
                ...
                <div className={style.right} style={{fontSize:"15px"}}>
                    ...
                    <div>
                        <span style={{fontSize:"0.9em"}}>{moment(this.props.data.time).fromNow()}</span>
                            {
                                this.state.liked ?
