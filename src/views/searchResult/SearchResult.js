...
class SearchResult extends React.Component {
              ...
              >
                ...
                <TableCell align="center">
                  {row.artists.length > 1
                    ? row.artists[0].name + "/" + row.artists[1].name
                    : row.artists[0].name}
                </TableCell>
                <TableCell align="right">{row.album.name}</TableCell>
                <TableCell align="right">
                  {formatSongDuration(row.duration)}
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
            ...
            {this.state.table3.map((item, index) => {
              ...
              return (
                <TableRow
                  key={item.id}
                  onDoubleClick={() => {
                    changeRouteToAlbum(this.props, item.id);
                  }}
                  hover={true}
                >
                  <TableCell>
          ...
          return (
            <LazyLoad height={250} once={true}>
              <div
                ...
                key={index}
                onClick={this.handleSelectVideoOrMV.bind(
                  this,
                  item.type,
                  item.vid
                )}
              >
                <img
                  src={createPicURL(item.coverUrl, 180, 160)}
                  alt=""
                  style={{ borderRadius: "10px" }}
                />
                <p style={{ fontSize: "14px", overflowWrap: "break-word" }}>
                  {item.title}
                </p>
          ...
          <TableBody>
            {this.state.table5.map((item, index) => (
              <TableRow
                ...
                hover={true}
                onDoubleClick={() => {
                  changeRouteToPlayList(this.props, item.id);
                }}
              >
                ...
                <TableCell component="th" scope="row" align="left">
                  <img
                    src={createPicURL(item.coverImgUrl, 50, 50)}
                    alt="pic"
  ...
  searchResult() {
    switch (this.state.currentIndex) {
      case 0:
        return this.tableForSingleSong();
      case 1: {
        return this.tableForSinger();
      }
      case 2: {
        return this.tableForAlbum();
      }
      case 3: {
        return this.tableForVideo();
      }
      case 4: {
        return this.tableForPlayList();
      }
      case 5: {
        return this.tableForUser();
      }
      default:
        return null;
    }
    ...
    return (
      <div style={{ marginLeft: "80px", marginTop: "30px" }}>
        ...
        <span style={{ fontSize: "15px", marginLeft: "20px" }}>
          {"共找到" + this.state.count + this.state.des}
        </span>
        <ThemeProvider theme={theme}>
          ...
          >
            ...
            <Tab
              label="专辑"
              value={2}
              onClick={this.handleStateChange.bind(this, "3")}
            />
            <Tab
              label="视频"
              value={3}
              onClick={this.handleStateChange.bind(this, "4")}
            />
            <Tab
              label="歌单"
              value={4}
              onClick={this.handleStateChange.bind(this, "5")}
            />
            <Tab
              label="用户"
              value={5}
              onClick={this.handleStateChange.bind(this, "6")}
            />
          </Tabs>
        </ThemeProvider>
        {this.state.loading ? (
