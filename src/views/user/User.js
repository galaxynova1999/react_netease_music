  ...
  render() {
    return (
      <div style={{ marginLeft: "5vw" }}>
        {!this.state.loading_top && (
          <div className={style.top}>
            <div>
              <img
                src={createPicURL(
                  this.state.detail.profile.avatarUrl,
                  200,
                  200
                )}
                alt="pic"
            ...
            <div style={{ height: "200px", marginLeft: "20px" }}>
              <div className={style.name}>
                <span style={{ fontSize: "20px" }}>
                  {this.state.detail.profile.nickname}
                </span>
                {parseInt(this.props.match.params.id) !== getUserID() ? (
                  <div style={{ marginLeft: "50px" }}>
                    {this.state.detail.profile.followed ? (
                      <Button variant="outlined">取消关注</Button>
                    ) : (
                      <Button variant="outlined">关注</Button>
                    )}
                  </div>
                ) : (
                  <div style={{ marginLeft: "120px" }}>
                    <Button variant="outlined">编辑个人资料</Button>
                  </div>
                )}
              </div>
              ...
              <div style={{ marginTop: "20px" }}>
                <p>
                  个人简介:
                  {this.state.detail.profile.signature
                    ? "暂无简介"
                    : this.state.detail.profile.signature}
                </p>
              </div>
        ...
        <div
          style={{ marginTop: "20px", marginBottom: "10px", fontSize: "20px" }}
        >
          <p>歌单</p>
          <Divider />
        </div>
