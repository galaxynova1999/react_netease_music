...
class PlayList extends React.Component {
            ...
            <TableBody>
              {this.state.songdetail.map((row, index) => (
                ...
                >
                  ...
                  <TableCell
                    onClick={() => {
                      changeRouteToAlbum(this.props, row.al.id);
                    }}
    ...
    switch (this.state.currentIndex) {
      ...
      case "2": {
        if (this.props.match.params.type === "songlist")
          ...
          if (
            this.state.detailForAlbum.description === null ||
  ...
  des() {
    let des;
    if (
      this.state.detailForPlayList.description === null ||
      this.state.detailForPlayList.description === ""
    ) {
      des = "暂无简介";
    } else if (this.state.detailForPlayList.description.length > 120) {
      des = this.state.detailForPlayList.description.substring(0, 120) + "...";
    } else des = this.state.detailForPlayList.description;
    return des;
  }
  tags() {
    if (this.state.detailForPlayList.tags.length === 0) {
      ...
      return this.state.detailForPlayList.tags.map(function (
        item,
        index,
        array
      ) {
    ...
    if (this.props.match.params.type === "songlist") {
      ...
      return (
        <>
          ...
          <p>
            时间:{" "}
            {moment(this.state.detailForAlbum.publishTime).format("YYYY-MM-DD")}
          </p>
        </>
      );
    }
          ...
          <div className={style.top}>
            ...
            <div className={style.info}>
              <div style={{ height: "60px" }}>
                ...
                {this.props.match.params.type === "songlist" && (
                  <div
                    style={{ float: "right", height: "30px", marginTop: "5px" }}
                  >
                    <span style={{ float: "left" }}>
                      歌曲数 {this.state.detailForPlayList.trackCount}
                    </span>
                    <Divider
                    ...
                    />
                    <span style={{ float: "left" }}>
                      播放数 {this.state.detailForPlayList.playCount}
                    </span>
                  </div>
                )}
              </div>
              {this.props.match.params.type === "songlist" && (
                <div style={{ display: "flex", alignItems: "center" }}>
                  <img
                    src={
                      this.state.detailForPlayList.creator.avatarUrl +
                      "?param=40y40"
                    }
                    alt=""
                  ...
                  <span style={{ marginLeft: "10px" }}>
                    {this.state.detailForPlayList.creator.nickname}
                  </span>
                  <span style={{ marginLeft: "20px" }}>
                    {moment(this.state.detailForPlayList.createTime).format(
                      "YYYY-MM-DD"
                    )}
                    创建
                  </span>
              ...
              <div style={{ marginTop: "20px" }}>
                ...
                {this.state.subscribed || this.state.isSub ? (
                  ...
                  >
                    已收藏(
                    {this.props.match.params.type === "songlist"
                      ? this.state.detailForPlayList.subscribedCount
                      : this.state.subCount}
                    )
                  </CollectedButton>
                ) : (
                  <UnCollectedButton
                    ...
                    onClick={this.handleCollect.bind(this)}
                    disabled={
                      this.state.detailForPlayList.creator.userId ===
                      getUserID()
                    }
                  >
                    收藏(
              ...
              <div style={{ marginTop: "20px" }}>{this.info()}</div>
            </div>
          </div>
        )}
        <div>
          <ThemeProvider theme={theme}>
            ...
            >
              <Tab
                label="歌曲列表"
                value="0"
                onClick={this.handleStateChange.bind(this, "0")}
              />
              <Tab
                label="评论"
                value="1"
                onClick={this.handleStateChange.bind(this, "1")}
              />
              <Tab
                label={
                  this.props.match.params.type === "songlist"
